...
Bomb::Bomb(float x, float y, color_t color)
{
    ...
    glm::vec3 v1(-0.026, 0.020, 0);
    glm::vec3 v2(0.026, 0.020, 0);
    glm::vec3 v3(0.0, 0.074, 0);
    ...
    for(int i = 0; i < base_size; i++){
        vertex_buffer_data[9+i] = trias[i];
    }
    for(int i = 0; i < 1*3*3; i++){
        vertex_buffer_data[i] = vertex_buffer_data_tmp[i];
    }
    // this->object = create3DObject(GL_TRIANGLES, 12*3, vertex_buffer_data, color, GL_FILL);
    ...
    base_size/=3;
    for(int i = 0; i < base_size*3+9; i+=3){
        color_buffer_data[i] = 178.0;
        color_buffer_data[i] /= 256.0;
        color_buffer_data[i+1] = 179.0;
        color_buffer_data[i+1] /= 256.0;
        color_buffer_data[i+2] = 170.0;
        color_buffer_data[i+2] /= 256.0;
    }
    for(int i = 0; i < 1*3*3; i+=3)
    {
        // Grey white
        color_buffer_data[base_size*3+i] = 79.9;
        color_buffer_data[base_size*3+i] /= 256.0;
        color_buffer_data[base_size*3+i+1] = 82.9;
        color_buffer_data[base_size*3+i+1] /= 256.0;
        color_buffer_data[base_size*3+i+2] = 4.9;
